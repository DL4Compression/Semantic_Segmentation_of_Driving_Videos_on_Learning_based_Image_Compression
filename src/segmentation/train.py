...
def get_arguments():
    ...
    parser.add_argument("--save_start",type=int, default=15000)
...
def main():
    ...
    # set models
    from SEG_DECODER import mydec
    deeplab = mydec.__dict__[args.arch](num_classes=args.num_classes)
    ...
    # set data loader
    data_set = Cityscapes(args.img_dir, args.lbl_dir, max_iters=max_iters, crop_size=input_size,
    ...
    it=0
    # start training:
    for ep in tq(range(epochs)):
        for i_iter, batch in enumerate(trainloader):
            images, labels = batch
            images = images.cuda()
            labels = labels.long().cuda()
            optimizer.zero_grad()
            lr = adjust_learning_rate(optimizer, args, it, epochs*len(trainloader))
            preds = model(images)
