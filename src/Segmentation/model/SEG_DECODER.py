...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.conv_1 = nn.Conv2d(16, 512, kernel_size=1, stride=1,
                     padding=0, bias=True)
    ...
    def forward(self, x):
        # print("Input:",x.shape)
        x = self.conv_1(x)
        # print("output of conv 1:",x.shape)
        x = self.layer3(x)
        # print("output of layer 3:",x.shape)
        if self.training:
            x_dsn = self.dsn(x)
            # print("dsn:",x_dsn.shape)
        x = self.layer4(x)
        # print("output of layer4:",x.shape)
        x = self.head(x)
        # print("output of head:",x.shape)
        if self.training:
            return [x, x_dsn]
        else:
            return [x]
